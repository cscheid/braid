...
braid tracks issues in a single CRDT document (automerge), synced through a
sync server. There is no git involvement: multiple agents on multiple
machines, branches, or worktrees can create, edit, and close issues in
parallel; replication and conflict resolution are automatic. Commands work
offline against a local cache and sync whenever the server is reachable.
...
braid finds its tracker via, in order (first hit wins):
...
- new tracker: `braid init` (then commit nothing — see the warning below)
- existing tracker: `braid init --join <doc-id>`

**The doc id is a secret.** It is a bearer token granting read *and* write
access to the tracker. Never commit `.braid.toml`; keep it gitignored.
Never print the doc id into logs, PRs, or issue text.
...
braid dep add "$new" <current-issue-id> --type discovered-from
```
...
| command | purpose |
|---|---|
| `braid ready [--json]` | active, unblocked issues — best starting point |
| `braid blocked [--json]` | active issues blocked by dependencies, with blockers |
| `braid list [--status S] [--json]` | all issues |
| `braid show <id> [--json]` | one issue (unique id fragments work: `braid show 6j42`) |
| `braid search <text> [--json]` | case-insensitive substring over titles, prose, labels, comments |
| `braid create <title> [flags]` | new issue; prints its id. Flags: `--description --type --priority --label --slug --assignee --json` |
...
| `braid reopen <id>...` | reopen closed issues |
...
| `braid init [--name N] [--join ID] [--sync-server URL] [--print-only]` | create or adopt a tracker |
...
- **dependency types**: `blocks`, `conditional-blocks`, `waits-for` make an
  issue unready while the target is open; `parent-child` expresses
...
character-wise; same scalar field, last writer wins; deleting an issue
